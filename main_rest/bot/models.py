...
from api.models import TrainerFit, DateLessonFit, LessonFit, UserFit
...
class MainTableAdmin(models.Model):
    ...
    lesson = models.ForeignKey(
        LessonFit, on_delete=models.CASCADE, verbose_name="Занятие"
    )
    trainer = models.ForeignKey(
        TrainerFit, on_delete=models.CASCADE, verbose_name="Тренер"
    )
    number_of_recorded = models.PositiveSmallIntegerField(
        default=0, verbose_name="Количество записанных на занятие"
    )
    max_number_of_recorded = models.PositiveSmallIntegerField(
        default=10, verbose_name="Макс. органичение по записи"
    )
    check_canceled = models.BooleanField(
        default=False, verbose_name="Отменить занятие?"
    )
    check_canceled_description = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="Причина отмены занятия"
    )
    check_change_description = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="Причина изменения занятия"
    )
    ...
    def clean(self):
        ...
        if (
            ...
            and not self.check_canceled
            and self.tracker.changed()
            and not self.check_change_description
        ):
            raise ValidationError(
                {
                    "check_change_description": "Причина изменения занятия обязательна, если данные (тренер, занятие "
                    "или дата) изменены."
                }
            )
...
class UserFitLesson(models.Model):
    ...
    lesson = models.ForeignKey(
        MainTableAdmin, on_delete=models.CASCADE, verbose_name="Занятие"
    )
